...
bool KRedisClient::Init(int nDBType)
{
    ...
    connectInfo.nPort = 6379;
    memcpy(connectInfo.szHost, "127.0.0.1", sizeof("127.0.0.1"));
...
bool KRedisClient::OnRequestSet(int nIndex, const KREQUEST_SET* pRequestSet)
{
    ...
    unsigned int uKeyLen = pRequestSet->uPrefixLen + pRequestSet->uKeyLen + REQUEST_KEY_UNDERLINED_LEN;
    unsigned int uValueLen = pRequestSet->uValueLen;
    pReply = RedisCommand(nIndex, "set %b %b", pRequestSet->data, (size_t)uKeyLen , pRequestSet->data + uKeyLen, (size_t)uValueLen);
    
    //落地到数据库里
    if(pReply!=NULL)
        //g_pDBClientMgr->PushMysqlRequest(nIndex + 1, (IKG_Buffer*)pRequestSet);
...
bool KRedisClient::OnRequestGet(int nIndex, const KREQUEST_GET* pRequestGet)
{
    ...
    pReply = RedisCommand(nIndex, "get %b", pRequestGet->data, uKeyLen - 1);
    if (pRequestGet->byHandleFlag & DB_FLAG_REDIS)
    {
...
bool KRedisClient::OnRequestHSet(int nIndex, const KREQUEST_HSET* pRequestHSet)
{
    ...
    pReply = RedisCommand(nIndex, "hset %b %b %b",
                          pRequestHSet->data, (size_t)pRequestHSet->uTableNameLen,
                          pRequestHSet->data + pRequestHSet->uTableNameLen, (size_t)pRequestHSet->uHashKeyLen,
                          pRequestHSet->data + pRequestHSet->uTableNameLen + pRequestHSet->uHashKeyLen, (size_t)pRequestHSet->uValueLen);
    //to do:落地到数据库
...
bool KRedisClient::OnRequestDel(int nIndex, const KREQUEST_DEL* pRequestDel)
{
    ...
    redisReply* pReply = NULL;
    IKG_Buffer* pCommonBuffer = NULL;
    size_t uKeyLen = pRequestDel->uKeyLen + pRequestDel->uPrefixLen + REQUEST_KEY_UNDERLINED_LEN;
    pReply = RedisCommand(nIndex, "del %b", pRequestDel->data, uKeyLen);

    //to do:同步数据库
    pCommonBuffer = GenCommonRespond(pRequestDel->lId, pRequestDel->byType, pReply);
    PushRespond(pCommonBuffer);
    bResult = true;
Exit0:
    if (pReply)
...
bool KRedisClient::OnRequestHDel(int nIndex, const KREQUEST_HDEL* pRequestHDel)
{
    ...
    redisReply* pReply = NULL;
    IKG_Buffer* pCommonBuffer = NULL;
    pReply = RedisCommand(nIndex, "hdel %b %b", pRequestHDel->data, (size_t)pRequestHDel->uTableNameLen, pRequestHDel->data + pRequestHDel->uTableNameLen, (size_t)pRequestHDel->uHashKeyLen);
    
    //to do:同步数据库
    pCommonBuffer = GenCommonRespond(pRequestHDel->lId, pRequestHDel->byType, pReply);
    PushRespond(pCommonBuffer);
    bResult = true;
Exit0:
    if (pReply)
